...
class HeartCurve extends THREE.Curve<THREE.Vector3> {
  // Explicit public constructor: THREE.Curve's constructor is protected in
  // current @types/three, so the implicit one would stay protected.
  constructor() {
    super();
  }
...
function RobotPrototype({ neckParams = { baseR: 0.25, baseH: -0.01, midR: 0.23, midH: 0.02, lipBottomR: 0.27, lipBottomH: 0.025, lipTopR: 0.28, lipTopH: 0.05, innerR: 0.24, innerDropH: 0.03 }, bodyParams = { bodyBevelR: 0.21, bodyBevelY: 0.38, bodyBevelT: 0.015 } }: { neckParams?: Record<string, number>, bodyParams?: Record<string, number> }) {
      ...
      <div className="w-full max-w-[1400px] mx-auto flex flex-col relative pointer-events-auto">
        ...
        <div className="flex flex-col lg:flex-row items-center justify-between relative gap-4 lg:gap-0">
          ...
          <div className="flex flex-wrap justify-center lg:justify-end items-center gap-2 sm:gap-3 w-full lg:w-auto mt-4 lg:mt-0 z-20">
            ...
            <button
              onClick={onCtaClick}
              className="px-5 sm:px-7 py-2.5 rounded-full bg-[#00ffc6] text-black text-xs sm:text-sm font-black hover:bg-[#00e5b2] transition-colors flex items-center gap-2 cursor-pointer shadow-[0_0_20px_rgba(0,255,198,0.4)]"
            >
              {ctaText}
...
export function RobotHero({
  backgroundText = 'DEUXEN AI',
  headline = 'Your AI receptionist.\nAlways on the line.',
  subheadline = 'Deuxen AI Automations builds voice agents that answer every call, book every appointment, and never put a customer on hold.',
  navItemsLeft = [
    ...
    >
      ...
      <div className="absolute inset-0 z-20 pointer-events-none flex flex-col">
        ...
        <div className="relative w-full max-w-[1400px] mx-auto px-8 flex-1 flex flex-col">
          ...
          <div className="mt-auto flex flex-col sm:flex-row justify-between items-start sm:items-end gap-6 pb-12 w-full">
            ...
            <motion.div
              ...
              className="max-w-md"
            >
              <h2 className="text-3xl sm:text-4xl font-bold tracking-tight text-zinc-900 whitespace-pre-line leading-[1.1]">
                {headline}
              </h2>
            ...
            <motion.div
              ...
              className="hidden sm:flex items-center gap-3 text-zinc-700"
            >
              <span className="relative flex h-2.5 w-2.5">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-500 opacity-75" />
                <span className="relative inline-flex rounded-full h-2.5 w-2.5 bg-emerald-500" />
              </span>
              <span className="text-sm font-semibold">Answering calls 24/7 — say hello, it's listening</span>
            </motion.div>
